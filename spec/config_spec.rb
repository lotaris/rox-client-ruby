...
describe RoxClient::Config do
  include Capture::Helpers
  include FakeFS::SpecHelpers
  Server ||= RoxClient::Server
  Project ||= RoxClient::Project

  let(:config){ RoxClient::Config.new }
  ...
  describe ".config" do
    ...
    before(:each){ RoxClient::Config.stub new: new_config }
    
    it "should create, load and memoize a configuration" do
      expect(new_config).to receive(:load).once
      3.times{ expect(RoxClient.config).to be(new_config) }
    end
  end
