...
describe RoxClient::Client do
  ...
  before :each do
    RoxClient::Cache.stub new: cache_double
    RoxClient::TestPayload.stub new: payload_double
    RoxClient::UID.stub new: uid_double
  end
  ...
  describe "when created" do
    ...
    it "should create an uid manager" do
      expect(RoxClient::UID).to receive(:new).with(workspace: WORKSPACE)
      RoxClient::Client.new server, client_options
    end
  end

  it "should upload the results payload" do
    expect(RoxClient::TestPayload).to receive(:new).with(run_double)
  ...
  describe "when the payload cannot be serialized" do
    before(:each){ payload_double.stub(:to_h).and_raise(RoxClient::PayloadError.new('bug')) }
  ...
  describe "when publishing fails due to a configuration error" do
    it "should output the error message to stderr" do
      server.stub(:upload).and_raise(RoxClient::Server::Error.new("bug"))
  ...
  describe "when publishing fails due to a server error" do
    it "should output the error message and response body to stderr" do
      server.stub(:upload).and_raise(RoxClient::Server::Error.new("bug", double(body: 'fubar')))
  ...
  describe "with payload caching enabled" do
    ...
    it "should output any error to stderr and not save the cache" do
      cache_double.stub(:load).and_raise(RoxClient::Cache::Error.new('bug'))
